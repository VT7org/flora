...
~ #This separator (~) must be between the text and buttons. Remove this comment too.  

button=[Duck, https://duckduckgo.com]  
button2=[Github, https://github.com]  

<b>NOTES -></b>  
Check /markdownhelp for more information on formatting and syntax.
"""
...
async def extract_userid(message, text: str):
    """Internal helper to resolve user ID from mention/username/ID"""
    def is_int(text: str):
        try:
            int(text)
        except ValueError:
...
async def extract_user_and_reason(message, sender_chat=False):
    ...
    try:
        if message.reply_to_message:
            ...
            reason = text.split(None, 1)[1] if len(args) >= 2 else None
            return id_, reason
...
async def time_converter(message: Message, time_value: str) -> Message | datetime:
    ...
    if check_unit == "m":
        temp_time = current_time + timedelta(minutes=int(time_digit))
    elif check_unit == "h":
        temp_time = current_time + timedelta(hours=int(time_digit))
    elif check_unit == "d":
        temp_time = current_time + timedelta(days=int(time_digit))
    else:
        return await message.reply_text("Incorrect time specified.")
    return temp_time
